    ...
    >
      ...
      <AdminPanel className="mb-4">
        <form method="GET" className="flex gap-3">
          ...
          {statusFilter && (
            <Link href="/admin/email-logs" className="h-9 px-4 border border-slate-200 text-sm rounded-md font-medium flex items-center hover:bg-slate-50">
              Réinitialiser
            </Link>
          )}
        </form>
        ...
        {totalPages > 1 && (
          <div className="mt-4 flex items-center justify-between text-sm text-slate-500">
            ...
            <div className="flex gap-2">
              {page > 1 && (
                <Link href={`/admin/email-logs?page=${page - 1}&status=${statusFilter}`}
