...
export async function GET(
  ...
  const [app, settings] = await Promise.all([
    prisma.inscriptionApplication.findFirst({
  ...
  }[app.candidatProfile] ?? app.candidatProfile;
  ...
  const siteName = settings.siteNameFr ?? "Centre de Qualification Professionnelle Maritime";

  const logoHtml = logoUrl
    ? `<img src="${logoUrl}" alt="${siteName}" style="height:56px;width:auto;object-fit:contain;" />`
    : `<div style="width:56px;height:56px;background:#0c1929;border-radius:8px;display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:18px;">C</div>`;
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header-left-text h1 { font-size: 18px; font-weight: 700; color: #0c1929; }
    .header-left-text p { font-size: 11px; color: #64748b; margin-top: 2px; }
...
<div class="page">
  ...
  <div class="header">
    <div class="header-left">
      ...
      <div class="header-left-text">
        <h1>${siteName}</h1>
        <p>Reçu d'inscription en ligne</p>
      </div>
    </div>
  ...
  <div class="footer">
    <span>${siteName} — Nador, Maroc</span>
